#!/usr/bin/env python3
"""
GDB Python script to test pollToFuture method with correct generic names.
"""
import sys
import os
...
try:
    ...
    print()
    print('=== Testing pollToFuture Method with Correct Generic Names ===')
    
    # Test cases with proper generic names from DWARF
    test_cases = [
        'reqwest::get::{async_fn#0}<&str>',
        'hyper::client::conn::http1::ready::{async_fn#0}<reqwest::async_impl::body::Body>',
        'h2::client::bind_connection::{async_fn#0}<hyper::common::io::compat::Compat<reqwest::connect::sealed::Conn>>',
        'tokio::time::sleep::{async_fn#0}',  # May not have generics
