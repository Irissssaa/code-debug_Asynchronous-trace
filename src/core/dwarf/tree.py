...
# Simple index class to replace QModelIndex functionality
class TreeIndex:
    ...
    def isValid(self):
        return self.row >= 0 and self.col >= 0 and self.data is not None
...
def load_children(parent_die: Union[DIE, DIEV1] , sort: bool): #(parent_die: Union[DIE, DIEV1] , sort: bool):
    ...
    if not hasattr(parent_die, "_children") or parent_die._children is None:
        ...
        try:
            ...
            print("Warning: This executable file is corrupt or incompatible with the current version of DWARF Explorer.")
            parent_die._children = []

class DWARFTreeModel:
    ...
    def createIndex(self, row, col, data):
        return TreeIndex(row, col, data)
    
    def beginResetModel(self):
        # Stub for Qt compatibility - no-op for library usage
        pass
    ...
    def data(self, index, role=None):
        ...
        # For library use, just return the display text
    ...
    def add_highlight(self, key, condition):
        if not self.highlight_condition:
            ...
            self.highlight_condition[key] = condition
        # Note: dataChanged.emit removed for library usage
